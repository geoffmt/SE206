...
public class CodeGenerator {
	...
	public void generateDagCode() {
		...
		for(Task t : _theDag.getOwnedTasks()) {
			for(Port p:t.getOwnedPorts()) {
				...
				if(p.getDataType().equals(PortDataType.FLOAT)) {
					_utils.addTextNewLine("float "+ p.getQualifiedName().replace(".", "_") + " = 0;");
				}
				else {
					_utils.addTextNewLine("int "+ p.getQualifiedName().replace(".", "_") + " = 0;");
				}
			}
		}
	...
	private void generateTaskCall(Task t) {
		...
		// parameters
		for(int i = 0; i < t.getOwnedPorts().size(); i ++) {
			Port p = t.getOwnedPorts().get(i);
			...
			_utils.addTextNoIdent(p.getQualifiedName().replace(".", "_"));
			if (i+1 < t.getOwnedPorts().size()) {
				_utils.addTextNoIdent(", ");
			}
		}
		...
		for(Port p:t.getOwnedPorts()) {
			...
			for(Channel c: _theDag.getOwnedChannels()) {
				if(	c.getSourceTask().equals(t) &&
					...
					_utils.addTextNewLine(c.getDestPort().getQualifiedName().replace(".", "_") + " = " + p.getQualifiedName().replace(".", "_") + ";");
	...
	private void generateTaskBody(Task t) {
		...
		for(Port p: t.getOwnedPorts()) {
			...
			if (p.getDataType().equals(PortDataType.FLOAT)) {
				_utils.addTextNewLine("*" + p.getName() + " += 0.1;");
			} else {
				_utils.addTextNewLine("*" + p.getName() + " += 1;;");
			}
		}
	}
